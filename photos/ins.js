      ...
      var render = function render(res) {
        ...
        for (var j = 0, len2 = res.list.length; j < len2; j++) {
          ...
          for (var i = 0, len = data.link.length; i < len; i++) {
            var minSrc = 'http://pc1z0iv7e.bkt.clouddn.com/' + data.link[i];
            var src = 'http://pc1zgp1z1.bkt.clouddn.com/' + data.link[i];
            var type = data.type[i];
            var target = src + (type === 'video' ? '.mp4' : '.jpg');
            src += '';
            var size = data.size[i][0] + 'x' + data.size[i][1];
    ...
    function(module, exports) {
      ...
      var initPhotoSwipeFromDOM = function initPhotoSwipeFromDOM(gallerySelector) {
        ...
        for (var i = 0, l = galleryElements.length; i < l; i++) {
          ...
          galleryElements[i].querySelector('ul').setAttribute('data-pswp-uid', i + 1);
